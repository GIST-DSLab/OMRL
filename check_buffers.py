...
def check_features():
    ...
    with h5py.File(f"{PATH}/data/macaw_offline_data/buffers_arc_train_0_sub_task_0.hdf5", 'r') as f:
        print(f.keys())
        obs_list = list(f["obs"])
        next_obs_list = list(f["next_obs"])
        for i in range(len(obs_list)):
            print("I:", i)
            print("obs")
            new_obs = obs_list[i].reshape(30, 30)
            print(new_obs)
            print("next_obs")
            new_next_obs = next_obs_list[i].reshape(30, 30)
            print(new_next_obs)
            if i == 5:
                break
